...
if "generated_data" not in st.session_state or st.session_state.generated_data.empty:
    st.info("""
    **ガイド ：** 左側のサイドバーで設定を行い、「ダミーデータを生成」ボタンを押してください。
    
    **設定項目の説明:**
    *   **商材・サービス概要**: ここに入力して「AIで商材を分析」を押すと、AIが商材特性に合わせて以下の数値を自動で推奨設定します。
    *   **想定CVR**: 目標とするコンバージョン率（%）。
    *   **滞在時間係数**: ユーザーの平均滞在時間の長さ（1.0が標準、数値が大きいほど長時間滞在）。
    *   **FV離脱率**: 最初の画面（ファーストビュー）で離脱するユーザーの割合（0.1〜0.9）。
    """)
    st.stop()
else:
